...
export function createGameListView(elements, handlers) {
  function render(state, history) {
    ...
    elements.historyList.replaceChildren(...history.map(entry => {
      ...
      const date = document.createElement('div'); date.className = 'h-date'; date.textContent = `${formatSavedAt(entry.savedAt)} · ${entry.games.length} ${entry.games.length === 1 ? 'гра' : 'ігор'}`;
